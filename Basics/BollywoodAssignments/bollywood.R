...
#You can write a command to find the maximum value of Ocollection, Wcollection, Fwcollecion and Tcollection using sapply
  ...
  sapply(bollywood[,c('Ocollection', 'Wcollection','Fwcollection','Tcollection')], function(x) max(x,na.rm = T))
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- sapply(colnames(bollywood[4:7]),function(targetcolumn){bollywood$Movie[which.max(bollywood[[targetcolumn]])] })
